...
==**注意通常由于复赛的测试集是不能下载的，它是当你提交后被自动挂载在镜像的根目录下，所以你在写预测脚本时只能假装存在一个这样的测试集目录，但如果线下在镜像里运行是会报错说找不到文件的，但是在页面提交后后台在执行的时候会挂载上这个目录，所以提交是可以正常运行的。应对思路是手动创造一个这样的环境(见末尾的小技巧)。**==
